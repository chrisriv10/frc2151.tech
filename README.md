...
6. After the first admin is authorized, use the **Manage Members** panel in `admin.html` to add or remove teammates by Firebase UID. The panel writes only through rules that require an existing admin, and it prevents the signed-in account from removing itself.
